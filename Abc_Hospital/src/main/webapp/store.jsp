...
<div class="mytabs">
    ...
    <label for="tabfree">Medicine List</label>
    <div class="tab">
...
<%!
        int medicineId;
        String medicineName,patientName,email,add,phone,ward;
%>
<%
	Class.forName("com.mysql.jdbc.Driver"); 
	Connection c=DriverManager.getConnection("jdbc:mysql://localhost:3306/hospdbms","root","mysql@123");
    PreparedStatement ps=c.prepareStatement("select * from store",ResultSet.TYPE_SCROLL_SENSITIVE,ResultSet.CONCUR_UPDATABLE);
...
%>   
             <tbody>
                    <tr>
                            ...
                            <td><%=ward%></td>
                            <td>
                            <button type="button" onclick="document.getElementById('<%=medicineId%>').style.display='block'" ><i class="fa fa-pencil" aria-hidden="true"></i></button>
                            <a  href="delete_nurse_validation.jsp?nurseId=<%=medicineId%>" onclick="return confirmDelete()" ><i class="fa fa-trash" aria-hidden="true"></i></a>
                            </td>
                    </tr>
            </tbody> 
<%
...
%>   
<div id="<%=medicineId%>" class="modal">
  
  <form class="modal-content animate" action="edit_nurse_validation.jsp" method="post">
    <div class="imgcontainer">
      <span onclick="document.getElementById('<%=medicineId%>').style.display='none'" class="close" title="Close Modal">&times;</span>
      <h3>Edit Ward Information</h3>
    </div>

        <div class="containerz">
        <p><b>Medicine Id</b></p>
        <input type="number" name="wardId" placeholder="Nurse Id" value="<%=medicineId%>" readonly="readonly">
        <br>
        <p><b>Medicine Name</b></p>
        <input type="text" name="wardName" value="<%=medicineName%>">
        <br>
    ...
    <div class="containerz" style="background-color:#f1f1f1">
      <button type="button" onclick="document.getElementById('<%=medicineId%>').style.display='none'" class="cancelbtn">Cancel</button>
...
<!----------------------popup end-------------------------------------------------->
    ...
    <label for="tabsilver">Add Medicine</label>
    <div class="tab">
                    ...
                    <p>Address</p>
                     ...
                     <p>Ward</p>
                        <input id="texting" type="text"   name="ward" placeholder="Ward" required="required">
                            ...
                            <input type="submit" class="psw" value="Add Medicine"></button>
                         </div>
                </form>
           <!--------------------------   Add Medicine Ends   --------------->   
    </div>
